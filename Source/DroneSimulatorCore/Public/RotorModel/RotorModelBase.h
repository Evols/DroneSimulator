...
struct FThrustSimBemtData {
  ...
  FThrustSimBemtData(double in_angle_of_attack,
                     const TArray<double> &in_reynolds, double in_v_induced,
                     double in_v_axial)
      : angle_of_attack(in_angle_of_attack), reynolds(in_reynolds),
        v_induced(in_v_induced), v_axial(in_v_axial) {}
};
...
struct FRotorSimulationResult {
  ...
  FRotorSimulationResult(
      const FThrustSimValue &in_value,
      const TOptional<FThrustSimAdditionalData> &in_additional_data,
      const FDebugLog &in_debug_log)
      : value(in_value), additional_data(in_additional_data),
...
public:
  virtual FRotorSimulationResult simulate_propeller_rotor(
      FSubstepBody *substep_body, double throttle,
      const FDronePropellerBemt *propeller, const FDroneMotor *motor,
      const FDroneBattery *battery, const FVector &propeller_location_local,
      bool is_clockwise, const USimulationWorld *simulation_world);
};
