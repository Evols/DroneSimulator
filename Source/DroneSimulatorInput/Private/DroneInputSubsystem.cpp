...
void UDroneInputSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
    ...
    this->calibrator = MakeShared<FDroneInputCalibrator>(this, this->profile_manager);
...
void UDroneInputSubsystem::Deinitialize()
{
    ...
    Super::Deinitialize();
    UE_LOG(LogDroneSimulatorInput, Log, TEXT("UDroneInputManager::Deinitialize - Input System Deinitialized"));
}

void UDroneInputSubsystem::Tick(float DeltaTime)
{
    // Also clear raw device data to prevent stuck values if device stops sending updates
...
static float apply_unsigned_axis_invert(float calibrated_value, bool is_inverted)
{
    return is_inverted ? 1.f - calibrated_value : calibrated_value;
}

static float apply_signed_axis_invert(float calibrated_value, bool is_inverted)
{
    return is_inverted ? -calibrated_value : calibrated_value;
}

static float compute_unsigned_throttle_value(float raw_value, const FAxisCalibrationData* calibration)
{
...
static float compute_signed_axis_value(float raw_value, const FAxisCalibrationData* calibration)
{
    if (calibration == nullptr)
    {
        return 0.f;
    ...
    if (!FMath::IsNearlyZero(range))
    {
        const float clamped_center = FMath::Clamp(calibration->zero_value, calibration->min_value, calibration->max_value);
        center_norm = FMath::Clamp((clamped_center - calibration->min_value) / range, 0.f, 1.f);
    }
...
static float compute_signed_throttle_value(float raw_value, const FAxisCalibrationData* calibration)
{
    ...
    // If using a controller with a zero value that's the min value as well, we're in a drone style controller,
    // with no auto centering
...
static float calibrate_axis_value(EDroneInputAxis axis, float raw_value, const FAxisCalibrationData* calibration, bool is_inverted, EThrottleCalibrationSpace throttle_calibration_space)
{
    if (axis == EDroneInputAxis::Throttle)
    {
        if (throttle_calibration_space == EThrottleCalibrationSpace::PositiveNegative)
        {
            const float signed_value = compute_signed_throttle_value(raw_value, calibration);
...
void UDroneInputSubsystem::handle_raw_analog_input(int32 device_id, FKey key, float analog_value)
{
    if (!calibrator.IsValid())
    {
        return;
    ...
    // Calibration Logic
    bool handled_by_calibration = calibrator->handle_raw_input(device_id, key, analog_value);

    // If calibration is active, we don't want to process the input for game control
    if (handled_by_calibration || calibrator->get_calibration_phase() != EDroneCalibrationPhase::None)
    {
        return;
    }
}

void UDroneInputSubsystem::update_raw_int_axis_value(int32 device_id, FName axis_name, int32 raw_value)
{
...
void UDroneInputSubsystem::register_device(int32 device_id, const FString& device_name, const FString& device_uid)
{
    // TODO: remove devices
    ...
    if (!was_present)
    {
        ...
        UE_LOG(LogDroneSimulatorInput, Log, TEXT("UDroneInputManager::RegisterDevice - New Device Registered: Id=%d, Name=%s"), device_id, *device.device_name);
        if (profile_manager.IsValid())
        {
            profile_manager->apply_saved_profile_if_any(device, device_id, device_mappings, calibration_data);
        }
    }
}
...
void UDroneInputSubsystem::set_axis_mapping(int32 device_id, FName axis_name, EDroneInputAxis game_axis)
{
    FInputAxisMapping& mapping = device_mappings.FindOrAdd(device_id).FindOrAdd(axis_name);
    mapping.input_device_axis_name = axis_name;
    mapping.game_axis = game_axis;
    UE_LOG(LogDroneSimulatorInput, Log, TEXT("UDroneInputManager::SetAxisMapping - Mapped DeviceId=%d, Axis=%s to GameAxis=%d"), device_id, *axis_name.ToString(), (int32)game_axis);
    if (profile_manager.IsValid())
    {
        if (const FDroneInputDevice* device = devices.Find(device_id))
        {
            const TMap<FName, FInputAxisMapping>* mappings = device_mappings.Find(device_id);
            const TMap<FName, FAxisCalibrationData>* calibrations = calibration_data.Find(device_id);
...
float UDroneInputSubsystem::get_calibrated_axis_value(EDroneInputAxis axis, EThrottleCalibrationSpace throttle_calibration_space) const
{
    if (!calibrator.IsValid() || calibrator->get_calibration_phase() != EDroneCalibrationPhase::None)
    {
        return 0.0f;
    }
    ...
    for (const auto& device_mapping_pair : device_mappings)
    {
        ...
        for (const auto& mapping_pair : mappings)
        {
            ...
            if (const FDroneInputDevice* device = devices.Find(device_id))
            {
                if (const float* raw_value = device->raw_axes.Find(mapping.input_device_axis_name))
                {
                    const FAxisCalibrationData* calibration = nullptr;
                    if (const TMap<FName, FAxisCalibrationData>* device_calibration = calibration_data.Find(device_id))
                    {
                        calibration = device_calibration->Find(mapping.input_device_axis_name);
                    }

                    return calibrate_axis_value(axis, *raw_value, calibration, mapping.is_inverted, throttle_calibration_space);
                }
            }
        }
    }
...
void UDroneInputSubsystem::remove_device_calibration(int32 device_id)
{
    ...
    UE_LOG(LogDroneSimulatorInput, Log, TEXT("UDroneInputSubsystem::remove_device_calibration - Removed calibration for DeviceId=%d"), device_id);
}

bool UDroneInputSubsystem::is_axis_inverted(int32 device_id, EDroneInputAxis axis) const
{
    if (const TMap<FName, FInputAxisMapping>* mappings = device_mappings.Find(device_id))
    {
        for (const auto& mapping_pair : *mappings)
        {
...
void UDroneInputSubsystem::set_axis_inverted(int32 device_id, EDroneInputAxis axis, bool inverted)
{
    if (TMap<FName, FInputAxisMapping>* mappings = device_mappings.Find(device_id))
    {
