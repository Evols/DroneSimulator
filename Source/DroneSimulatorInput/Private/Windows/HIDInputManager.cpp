    ...
    for (int32 i = devices.Num() - 1; i >= 0; --i)
    {
        ...
        while (reads < max_reads)
        {
            ...
            if (status == EHIDPollStatus::Success)
            {
                ...
                if (input_subsystem)
                {
                    input_subsystem->register_device(device_data.device_id, device_data.device_name, device_data.device_uid);
                    ...
                    for (const auto& pair : device_data.raw_axes)
                    {
                        ...
                        input_subsystem->handle_raw_analog_input(device_data.device_id, axis_key, pair.Value);
                    }

                    // Raw Ints
                    for (const auto& pair : device_data.raw_int_axes)
                    {
                        input_subsystem->update_raw_int_axis_value(device_data.device_id, pair.Key, pair.Value);
                    }
                }
            }
            ...
            {
                // Device Disconnected
                UE_LOG(LogDroneSimulatorInput, Warning, TEXT("Device %d Disconnected (Poll Error)"), device->get_internal_id());
...
void FHIDInputManager::enumerate_devices(UDroneInputSubsystem* input_subsystem)
{
#if PLATFORM_WINDOWS
    GUID hid_guid;
    HidD_GetHidGuid(&hid_guid);

    HDEVINFO device_info_set = SetupDiGetClassDevs(&hid_guid, nullptr, nullptr, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
    if (device_info_set == INVALID_HANDLE_VALUE) return;
    ...
    while (SetupDiEnumDeviceInterfaces(device_info_set, nullptr, &hid_guid, device_index++, &device_interface_data))
    {
        ...
        if (required_size > 0)
        {
            PSP_DEVICE_INTERFACE_DETAIL_DATA detail_data = (PSP_DEVICE_INTERFACE_DETAIL_DATA)FMemory::Malloc(required_size);
            detail_data->cbSize = sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA);

            if (SetupDiGetDeviceInterfaceDetail(device_info_set, &device_interface_data, detail_data, required_size, nullptr, nullptr))
            {
                ...
                if (!exists)
                {
                    ...
                    TSharedPtr<FWindowsHIDDevice> new_device = MakeShared<FWindowsHIDDevice>(new_id, device_path);
                    if (new_device->open())
                    {
    ...
    for (int32 i = devices.Num() - 1; i >= 0; --i)
    {
        ...
        {
            UE_LOG(LogDroneSimulatorInput, Warning, TEXT("Device %d Disconnected (Enumeration)"), devices[i]->get_internal_id());
            if (input_subsystem)
            {
                input_subsystem->handle_device_disconnected(devices[i]->get_internal_id());
            }
            devices[i]->close();
            devices.RemoveAt(i);
        }
