 ...
 */
struct DRONESIMULATOR_API FDroneAirfoilTable
{
	...
	FDroneAirfoilTable(TArray<float> InReynoldsNumbers, TArray<FReynoldsEntry> InReynoldsEntries)
		: reynolds_numbers(MoveTemp(InReynoldsNumbers))
		, reynolds_entries(MoveTemp(InReynoldsEntries))
	{
	}
	...
	bool is_valid() const
	{
		return reynolds_numbers.Num() > 0
			&& reynolds_entries.Num() == reynolds_numbers.Num()
			&& reynolds_entries.Num() > 0
			&& reynolds_entries[0].angles_of_attack.Num() > 0;
	}

	static FDroneAirfoilTable from_airfoil_asset_table(const UDroneAirfoilAssetTable* asset);
};

struct DRONESIMULATOR_API FDroneAirfoilSimplified
{
	...
	static FDroneAirfoilSimplified from_airfoil_asset_simplified(const UDroneAirfoilAssetSimplified* asset);
};

using FDroneAirfoil = TUnion<FDroneAirfoilTable, FDroneAirfoilSimplified>;

/**
 * Inside simulation, we want to work with SI units (meters, kilograms, seconds, Newtons) and radians.
 *
 * For configuration, we display easy to understand units (inches, cm, kg) and degrees.
 * Unreal uses centimeters and centi-Newtons (kg·cm/s²).
 */
USTRUCT(BlueprintType)
struct DRONESIMULATOR_API FDronePropellerBemt
{
	GENERATED_BODY()
...
USTRUCT(BlueprintType)
struct DRONESIMULATOR_API FDronePropellerSimplified
{
	GENERATED_BODY()
 ...
 */
USTRUCT(BlueprintType)
struct DRONESIMULATOR_API FDroneFrame
{
	GENERATED_BODY()

public:

	// Distance in each axis from the center of the drone, in unreal-units (centimeters).
	UPROPERTY(BlueprintReadWrite)
 ...
 */
USTRUCT(BlueprintType)
struct DRONESIMULATOR_API FDroneMotor
{
	GENERATED_BODY()
 ...
 */
USTRUCT(BlueprintType)
struct DRONESIMULATOR_API FDroneBattery
{
	GENERATED_BODY()
