...
#include "DroneSimulatorGame/DroneSimulatorGame.h"
...
void ADroneFlightRecordingManager::save_flight_record_asset()
{
	...
	{
		UE_LOG(LogDroneSimulatorGame, Warning, TEXT("Trying to save flight record asset but it is not created yet"));
		return;
	}
	...
	if (package == nullptr)
	{
		UE_LOG(LogDroneSimulatorGame, Warning, TEXT("Trying to save flight record asset with no package"));
		return;
	}
